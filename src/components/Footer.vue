<template>
	<footer
		class="bg-brandNeutral-700 px-4 xl:px-8 h-[83vh] mobile375:h-[73vh] mobile390:h-[60vh] mobile411:h-[65vh] md:h-[65vh] lg:h-[65vh] responsiveScreen1360:h-[68vh] xl:h-[70vh] responsiveScreen1440:h-[68vh] responsiveScreen1495:h-[68vh] responsiveScreen1550:h-[68vh] responsiveScreen1610:h-[68vh] responsiveScreen1680:h-[68vh] 2xl:h-[60vh] 3xl:h-[43vh]">
		<div class="flex justify-center items-center mt-4 xl:mt-8">
			<p
				class="font-clashDisplay font-medium text-center text-2xl md:text-4xl xl:text-6xl text-brandPrimaryWhite leading-none mt-24">
				feel free<br />
				to reach out
			</p>
		</div>
		<div
			class="flex justify-center items-center gap-8 md:gap-12 text-xs md:text-base text-brandSecondaryGreen font-satoshi font-bold mt-8 xl:mt-16">
			<a href="https://www.github.com/yangedruce" target="_blank">Github</a>
			<a href="https://www.behance.net/yangedruce" target="_blank">Behance</a>
			<a href="https://www.linkedin.com/in/yangedruce/" target="_blank">LinkedIn</a>
			<a href="mailto:hello@yangedruce.com" target="_blank">Email</a>
		</div>
		<div class="flex flex-col md:flex-row justify-between items-center md:h-16 md:pt-0 mb-32 xl:mb-52 mt-8">
			<p class="font-satoshi text-center md:text-start text-xs md:text-base text-brandNeutral-500">
				© 2015–2023 Yang Edruce.
				<br />
				All rights are reserved.
			</p>
			<p class="font-satoshi text-center md:text-end text-xs md:text-base text-brandNeutral-500 mt-4 md:mt-0">
				Designed and developed by
				<br />
				Yang Edruce.
			</p>
		</div>
	</footer>
</template>

<script></script>
